...
std::mutex PrivacyIdInfo::m_initializeMutex;
...
{
	std::lock_guard < std::mutex > guard(m_initializeMutex);
	if(m_isInitialized)
	...
	while ( (res = sqlite3_step(pStmtPrivilege.get())) == SQLITE_ROW )
	{
		...
		if (feature != NULL)
		{
			...
			res = isFeatureEnabled(std::string(feature), isSupported);
...
int
PrivacyIdInfo::getPrivaycDisplayNameStringId(const std::string privacyId, std::string& displayNameStringId)
{
	if (!m_isInitialized)
		initialize();
	...
	if ( sqlite3_step(pStmt.get()) == SQLITE_ROW )
	{
		...
		if (pNameId == NULL)
			displayNameStringId = privacyId;
		else
			displayNameStringId = pNameId;
	}
	else
	{
		LOGI("Cannot find privacy string %s ", privacyId.c_str());
...
int
PrivacyIdInfo::getPrivaycDescription(const std::string privacyId, std::string& description)
{
	if (!m_isInitialized)
		initialize();
	...
	if ( sqlite3_step(pStmt.get()) == SQLITE_ROW )
	{
		...
		description = std::string(dgettext(pModuleId, pNameId));
	}
	else
	{
		LOGI("Cannot find privacy string %s ", privacyId.c_str());
...
int
PrivacyIdInfo::getPrivaycDescriptionStringId(const std::string privacyId, std::string& descriptionStringId)
{
	if (!m_isInitialized)
		initialize();
...
int
PrivacyIdInfo::isFeatureEnabled(const std::string feature, bool& enabled)
{
	int res = 0;
	...
	if(feature.find("tizen.org/feature") != std::string::npos)
	{
		res = system_info_get_platform_bool(feature.c_str(), &enabled);
	}
	else
	{
		res = system_info_get_custom_bool(feature.c_str(), &enabled);
	}
	TryReturn(res == 0, PRIV_MGR_ERROR_SYSTEM_ERROR, , "system_info_get : %d", res);
